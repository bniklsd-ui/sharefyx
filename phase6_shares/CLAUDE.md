...
**[2026-08-25, P7 Step C3] Dieser Seam ist geschlossen, nicht mehr offen.** Zeile 16 oben
bleibt als Herkunftsnachweis stehen (der Stub selbst wurde hier gebaut), ist aber überholt:
`config.py :: UiSettings.space_admin_enabled` ist jetzt `True` per Default, `app.html`s
Menüpunkt trägt kein `disabled`/„kommt in Phase 7" mehr, `test_app_html_has_a_disabled_
manage_spaces_stub` existiert nicht mehr (ersetzt durch `test_app_html_has_a_live_manage_
spaces_entry`). Details, Code, neue Tests: `phase7_spaces_admin/CLAUDE.md` Step C3.
