...
- [phase6_shares/CLAUDE.md](../phase6_shares/CLAUDE.md) — 🔄 ~32KB · phase head; **Block A (Steps 0–3) vollständig gebaut** (Werkzeuge/Betrieb/Update-Banner — Details im Archiv, siehe unten), **GATE A→B nur noch Punkt 3 offen** (Purge-Zeilenrückgang, frühestens 2026-08-28); Steps 4–6 (Storage-Fundament, Rechtepolitik, Verwaltung/Migration, Block B) **✅ gebaut**; **[2026-08-13] Steps 4–6 live deployed** (`main`@`d068d1c`) — Cutover auf `SharePolicy` vollzogen, `niklas`↔`fabian` gegenseitiges Lesen vorher per `.share.yml` gesichert, eine Richtung über den echten Connector live-verifiziert (fabian→niklas offen), neuer Shared Space `IT-Sekus-Projekt` angelegt; **UI-Fund, zwei Teile:** Space-Übersicht zeigte geteilte Spaces als „nur lesen" trotz Schreibrecht — Teil 1 (Badge) deployed + live bestätigt, Teil 2 (Anlegen-Knopf/Text innerhalb des Spaces, `app.js :: ownSpaceActive()` an acht Stellen) behoben, noch nicht deployed; **Planungsvormerkung für die nächste Session (Opus):** Item-Verschieben zwischen Ordnern/Spaces, fehlt auf allen drei Schichten, Scoping-Notizen im Session-Block; read + newest Session-stopped block first
