...
updated: 2026-08-11 (Gate-A→B-Punkt 4 vollständig live bestanden, nur noch Punkt 3 offen)
---
# Doc Index (L0)
...
- [phase6_shares/CLAUDE.md](../phase6_shares/CLAUDE.md) — 🔄 ~28KB · phase head; Step 0 ✅ (Haushalt, Verifikationsdurchlauf V39–V41, Regeländerungen §0.7 a/b/c) + Step 1 ✅ (Werkzeug-Ergonomie — `patch_item`, Quittungen statt Volltext, `mcp_smoke.py` 13/13) + Step 2 ✅ gebaut/Live-Teile beim Nikinger (Betrieb — O2 geschlossen, `ua`-Feld/V42 Fenster ab 2026-08-10, `diagnose.sh` Prüfung 11, `ui_budget.py`-Latenzmessung) + Step 3 ✅ gebaut (Update-Log und Banner — Schema 3, `webui/updates.py`, `deploy.sh`-Gate; **Gate-A→B-Punkt 4 seit 2026-08-11 vollständig live bestanden**, Banner + Fabians Bestätigung) — **Block A (Steps 0–3) vollständig gebaut, GATE A→B nur noch Punkt 3 offen** (Purge-Zeilenrückgang, frühestens 2026-08-28); read + newest Session-stopped block first
