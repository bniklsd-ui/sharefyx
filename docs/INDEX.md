...
- [phase6_shares/CLAUDE.md](../phase6_shares/CLAUDE.md) — 🔄 ~33KB · phase head; Step 0 ✅ (Haushalt, Verifikationsdurchlauf V39–V41, Regeländerungen §0.7 a/b/c) + Step 1 ✅ (Werkzeug-Ergonomie — `patch_item`, Quittungen statt Volltext, `mcp_smoke.py` 13/13) + Step 2 ✅ gebaut (Betrieb — O2 geschlossen, `ua`-Feld, **V42 geschlossen 2026-08-12** — unterscheidet nicht zwischen Claude-Oberflächen, `diagnose.sh` Prüfung 11, `ui_budget.py`-Latenzmessung) + Step 3 ✅ gebaut (Update-Log und Banner — Schema 3, `webui/updates.py`, `deploy.sh`-Gate; **Gate-A→B-Punkt 4 seit 2026-08-11 vollständig live bestanden**, Banner + Fabians Bestätigung) — **Block A (Steps 0–3) vollständig gebaut, GATE A→B nur noch Punkt 3 offen** (Purge-Zeilenrückgang, frühestens 2026-08-28); **[2026-08-12]** Step 4 (Storage-Fundament, Block B) per Nikinger-Entscheidung vor Gate-Abschluss begonnen (Punkt 3 bleibt offen, mitgetragen) — Charakterisierung (P6-D) grün, Umbau läuft; read + newest Session-stopped block first
