...
- [phase5_ui/CLAUDE.md](../phase5_ui/CLAUDE.md) — 🔄 ~34KB · phase head; Step 0 ✅, Step 1 ✅ (S2–S8 geschlossen), Step 2 ✅ (Schema 2, `UserDirectory`, Migrationsskript), Step 3 ✅ (neues Paket `phase5_ui/webui`, Sessions/CSRF/Login/Logout), Step 4 ✅ (Selbstverwaltung: Einladung, Passwort, TOTP, Recovery, Connectoren; neuer Sicherheitsbefund S9 gefunden+geschlossen), `/ui/*` aus Step 5 vorgezogen in `mcpserver/app.py` gemountet (Block-A-Gate war sonst nicht live durchführbar); 🟡 ein Live-Fund beim Gate noch offen (`/ui/enroll/confirm`: `Referrer-Policy: no-referrer` nullte den `Origin`-Header auf reinen Form-POSTs, `require_csrf()` lehnte korrekt ab; Fix auf `strict-origin` umgesetzt, Live-Bestätigung durch den Nikinger aussteht); read + newest Session-stopped block first
