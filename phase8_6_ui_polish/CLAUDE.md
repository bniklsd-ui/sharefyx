...
0. **Entscheidung Nikinger — Plugin-Rückbau ausführen?** Einzeiler, reversibel, im
   User-Scope (nicht im Repo): `rm ~/.config/opencode/plugins/opencode-vision.js`
   + OpenCode-Neustart. Gegenprobe danach: `opencode run -f <png> -- "<Pixel-Frage>"`
   muss **ohne** Tool-Call korrekt antworten. Alternative, falls M2.x je gebraucht
   wird: in `~/.config/opencode/opencode-vision.json` `"models"` von `["*"]` auf
   `["*/MiniMax-M2*"]` verengen. Begründung + Messreihe:
   `docs/concepts/sichtpruefung_automation_tooling.md` §Messbefund 2026-09-11.
...
**Offene Entscheidung für den Nikinger (ersetzt die Frage aus dem V121/V122-Block, die sich
erledigt hat — der Plugin-Pfad kann §4 nie aktivieren):** Soll ich den Rückbau ausführen?
`rm ~/.config/opencode/plugins/opencode-vision.js` + OpenCode-Neustart. Reversibel, im User-Scope,
außerhalb des Repos — deshalb nicht selbst gemacht. Gegenprobe danach:
`opencode run -f <png> -- "<Pixel-Frage>"` muss **ohne** Tool-Call korrekt antworten.
