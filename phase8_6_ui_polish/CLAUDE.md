...
### Nächster Schritt: **Block J (der `pytest`-Flake, P8.6-AJ)**

Plan §6: `crypto.new_public_id()` (Rejection-Sampling gegen führendes `-`, 1,569 %
gemessen) + zwei Aufrufe in `store.py:294/393`; `authctl.py:199` bekommt einen
`help`-Text für den Altbestand. Drei neue Tests
(`test_new_public_id_never_starts_with_a_dash` + zwei weitere) — Erwartung
`pytest ≥ 972 passed, 0 failed` (das `+1` vom Flake wird zur festen Zahl).
Datierte Tabu-Ausnahme (P8.6-AJ) für `phase4_auth/authserver/{crypto.py,store.py}`
genau zwei Dateien, `store.py` genau 2 Zeilen — wird vor dem Commit mit
`git diff --stat -- phase4_auth/authserver` enger geprüft als der Standard-Tabu-Diff
(Plan §6.4).
