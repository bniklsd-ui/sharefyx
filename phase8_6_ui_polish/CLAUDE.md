...
| 10 | **Block E — messen, nicht bauen** (Befund 9): `p86_viewport_probe.py` (CDP, `getBoundingClientRect` + `getComputedStyle` + `elementFromPoint` auf jeden Knopf-Mittelpunkt) bei 1024/1200/1440 px, **zweimal** — gegen die Wegwerf auf aktuellem `main` **und lesend gegen die Produktion**. Trennt **9a** (live auf `v3.0.1`, Ursache unbekannt) von **9b** (durch Block C eingeführt: `overflow: hidden` + `flex: 1` schlägt `height: auto`). **E2a erledigt 2026-09-14:** Befund **9b banner-abhängig**, nicht unconditional — bei 1200 px **mit** Update-Banner zwei Recent-Items (`DB-Migration skript`, `Smoke-Tests ausbauen`) vom `.overview__col-right`-Container überdeckt; **ohne** Banner kein sichtbar unerreichbarer Knopf. **9a wartet** auf E2b gegen Produktion (URL + Creds vom Nikinger, Hard-Rule-9-konform: kein `pkill`, kein `systemctl`). **V125** (gleiche Ursache 9a/9b?) **partiell**: 9b ist nicht die vom Plan angenommene reine CSS-Wanze, sondern Banner-getrieben (Banner 140 px → Detail-Overview 761 px → col-left auto-Row 284 px → 5. Item passt nicht). **V126** (1024 px unerreichbar?) **erledigt**: **0** sichtbare Buttons blockiert, alle 53 hidden-button-Treffer haben `rect=(0,0,0,0)` (data-view="list" blendet die Detail-Buttons aus — separater Mechanismus, kein Layout-Bug). | E | 🟡 (E2a done, E2b wartet auf URL + Creds) | +1 Skript (`p86_viewport_probe.py`, 421 Z. / 16 KB), +6 Screenshots `p86_probe_{main,main-clean}_{1024,1200,1440}.png`, +2 Probe-JSON `phase8_6_ui_polish/probes/e2a_{main,main_clean}.json` (~65 KB je), kein Produktcode, Tabu-Diff §0.3 leer |
