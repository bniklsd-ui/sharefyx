...
| P8-22 (W) | 200-Knoten-Wegwerf-Datensatz: Simulation kommt < 3 s zur Ruhe, Interaktion ohne Hakeln; `prefers-reduced-motion` rendert statisch | ⬜ | **nicht gefahren** — D2-Wegwerf hat 14 Knoten; 200-Knoten-Fixture + Smoke-Lauf = eigener Setup (`graph.js` für 200 Knoten, künstliche `links:`-Kanten, Playwright-Timing-Assertion); `prefers-reduced-motion`-Pfad existiert in `graph.js` (300-Ticks-Sync), ohne Browser-Verifikation |
...
| P8-24 (W) | Playwright-Durchlauf gegen Wegwerf: Übersicht → Scope → Graph → Knotenklick → Item | 🟡 | drei vorhandene Wegwerf-Smokes decken Teilpfade ab (`d1_smoke` Space-Chip-Klick, `d2_smoke` Graph+Zoom, `c4c5_smoke` Design); **ein kombinierter End-to-End-Smoke existiert nicht** |
...
- **9 🟡:** P8-5 (A3 Restdefekt), P8-8 (B3 Zweitnutzer-Pass-Through fehlt),
  P8-14/15/16 (C1–C5 Sichtprüfungen am echten Gerät), P8-18/19 (D1 Sichtprüfung 2),
  P8-20/21 (D2 Drag/Cutoff ohne Browser-Assertion), P8-23 (D3 vorbereitet, post-deploy),
  P8-24 (voller E2E-Smoke fehlt).
- **2 ⬜:** P8-22 (200-Knoten-Wegwerf-Datensatz + Smoke nie gefahren).

**Was §9 (Phase-8-Closeout, P8-N) noch braucht (gesammelt, keine Auslagerung in diese
Sitzung):**

- **200-Knoten-Wegwerf-Fixture + Smoke-Lauf** für P8-22 (Skript + Daten-Generator,
  eigener Port, Playwright-Timing-Assertion).
- **Kombinierter `phase8_e2e_smoke.py`** für P8-24 (Übersicht → globaler Scope → Graph
  → Knotenklick → Editor öffnet) gegen einen aktuellen D2-Wegwerf.
...
- **§9-Restdefektabschnitt** mit den drei benannten Punkten (A3 Klammer/Aufzählung,
  Item-Link-Picker-Body-Lücke, Picker-A11y `aria-selected`).
