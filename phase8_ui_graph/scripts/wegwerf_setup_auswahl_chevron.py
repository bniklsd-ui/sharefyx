#!/usr/bin/env python3
"""Phase 8 Block C -- Auswahl-Boxen Chevron-Vorbild -- Wegwerf-Instanz aufsetzen.

Steht im Standing-Permission-Rahmen der Phase (CLAUDE.md Wurzel + Phase 8 §0.0):
eigener Port (18771), tmp-DATA_ROOT, File-Keyring-Backend statt `nikinger-space`,
...
Datenlage identisch zum C4+C5-Smoke: 5 eigene Items + 2 fremde = 7 Items ueber
zwei Spaces. Der Smoke-Lauf prueft strukturell, ob die Chevron-Regel auf alle
sieben `<select class="input">`-Vorkommen greift:
  - #field-status (Item-Editor, Status)
  - #create-type (Anlegen-Dialog, Item-Typ)
  - #new-folder-parent-select (Anlegen-Dialog, Uebergeordneter Ordner)
  - #move-space-select (Verschieben-Dialog, Ziel-Space)
  - #move-folder-select (Verschieben-Dialog, Ziel-Ordner)
  - #space-member-write-select (Space-Verwaltung, lesen/schreiben)
  - Per-Item-Share-Row (`<select class="input">` dynamisch erzeugt in dialogs.js:421)
Visuelle Feinjustierung (Chevron-Groesse, Stroke-Width, Position) macht die
Nikinger-Sichtpruefung am echten Geraet, nicht dieser Lauf.
"""
...
WEGWERF_ROOT = Path("/tmp/opencode/sharefyx-wegwerf-auswahl-chevron")
...
def _provision_user(space: str) -> dict[str, str]:
    ...
    password = "wegwerf-chevron-" + secrets.token_urlsafe(8)
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    env["SPACE_PUBLIC_BASE_URL"] = "https://wegwerf-chevron.invalid"
    ...
    proc = subprocess.Popen(
        [".venv/bin/python", "phase2_mcp/scripts/serve.py",
         ...
         "--allowed-host", "wegwerf-chevron.invalid"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Wegwerf-Instanz Phase 8 Chevron-Vorbild")
