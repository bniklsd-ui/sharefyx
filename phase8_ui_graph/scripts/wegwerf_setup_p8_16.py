#!/usr/bin/env python3
"""Phase 8 P8-16 -- Wegwerf-Instanz fuer die Glass-Fallback-Probe.

Steht im Standing-Permission-Rahmen der Phase (CLAUDE.md Wurzel + Phase 8 §0.0):
eigener Port (18775, 18770 war C4+C5, 18772 war 200-Knoten, 18773 war v3ritt),
tmp-DATA_ROOT, File-Keyring-Backend statt `nikinger-space`, User direkt in
auth.sqlite3 provisioniert. Cleanup per `kill -TERM $(cat serve.pid)` -- kein
`pkill -f` mit Regex (Hard Rule 9).

Bewusst klein gehalten: P8-16 braucht nur Glass-Träger (.list__head auf jeder
Listen-Ansicht, .overlay__panel auf jedem Dialog/Modal), keine 200 Knoten,
kein Graph, kein Force-Toggle. Eine Space, ein User, ein Item reicht, um die
vier Glass-Träger .list__head/.overlay__panel/.update-banner/.toast im Browser
zu beobachten.
"""
...
WEGWERF_ROOT = Path("/tmp/opencode/sharefyx-wegwerf-p816")
...
PORT = 18775
OWN_SPACE = "alpha"
PUBLIC_BASE_URL = "https://wegwerf-p816.invalid"
...
def _provision_user(space: str) -> dict[str, str]:
    ...
    password = "wegwerf-p816-" + secrets.token_urlsafe(8)
...
def cmd_seed_items(args: argparse.Namespace) -> int:
    ...
    _seed_items(OWN_SPACE, [
        ("Notiz fuer P8-16-Probe", "note"),
    ])
    print(f"Item angelegt (own={OWN_SPACE}).")
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    env["SPACE_PUBLIC_BASE_URL"] = PUBLIC_BASE_URL
    ...
    proc = subprocess.Popen(
        [".venv/bin/python", "phase2_mcp/scripts/serve.py",
         ...
         "--allowed-host", "wegwerf-p816.invalid"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Wegwerf-Instanz Phase 8 P8-16")
