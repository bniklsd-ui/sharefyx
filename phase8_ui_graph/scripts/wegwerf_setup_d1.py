#!/usr/bin/env python3
"""Phase 8 D1 -- Wegwerf-Instanz aufsetzen, starten, stoppen, aufraeumen.

Reproduziert das C3-Muster (P5 Step 6/7b standing permission) und ergaenzt es um einen zweiten
Space `beta` plus share_read von `alpha`, damit die Uebersichts-Seite sowohl eine eigene Zeile
(Kategoriepunkt own) als auch eine fremde Zeile (Kategoriepunkt foreign) rendert -- genau das,
was D1 sichtbar machen muss.

Eigener Port (18767), eigenes DATA_ROOT (tmp), eigene auth.sqlite3, eigener File-Keyring
(Hard Rule 1: niemals echte Geheimnisse in den realen `nikinger-space`-Keyring schreiben).
Stopp per `kill -TERM $(cat serve.pid)` -- kein `pkill -f` (Hard Rule 9).

Aufruf:
    python phase8_ui_graph/scripts/wegwerf_setup_d1.py setup
    python phase8_ui_graph/scripts/wegwerf_setup_d1.py seed-items
    python phase8_ui_graph/scripts/wegwerf_setup_d1.py start
    python phase8_ui_graph/scripts/wegwerf_setup_d1.py stop
    python phase8_ui_graph/scripts/wegwerf_setup_d1.py cleanup
"""
...
class FileBackend(keyring.backend.KeyringBackend):
    """Minimal-Keyring-Backend (siehe wegwerf_setup_c3.py fuer die Begruendung)."""
...
def _provision_user(space: str) -> dict[str, str]:
    ...
    password = "wegwerf-d1-" + secrets.token_urlsafe(8)
...
def _create_foreign_space(space: str) -> None:
    """Legt einen zweiten Space `beta` an und gibt `alpha` Leserecht darauf -- so erscheint
    `beta` in `alpha`s Overview als foreign (Kategoriepunkt foreign, slate)."""
    env = os.environ.copy()
    env["SPACE_DATA_ROOT"] = str(DATA_ROOT)
    subprocess.run(
...
def cmd_seed_items(args: argparse.Namespace) -> int:
    _install_keyring()
    # Reihenfolge ist wichtig: erst den fremden Space anlegen (sonst scheitert _seed_items
    # darunter), dann Items in beide Spaces saeen, dann share_read setzen.
    _create_foreign_space(FOREIGN_SPACE)
    # Eigener Space: 3 Items (gemischt Typen, damit Counter-Chips nicht-leer sind).
    _seed_items(OWN_SPACE, [
        ("Erste Notiz", "note"),
        ("Aufgabe fuer morgen", "task"),
        ("Bezug zu Phase 8", "note"),
    ])
    # Fremder Space (Leserecht von alpha): 2 Notizen -- foreign-Zeile mit 2 Chips.
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    env["SPACE_PUBLIC_BASE_URL"] = "https://wegwerf-d1.invalid"
    ...
    proc = subprocess.Popen(
        [".venv/bin/python", "phase2_mcp/scripts/serve.py",
         ...
         "--allowed-host", "wegwerf-d1.invalid"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Wegwerf-Instanz D1")
