...
detail: L3
up: CLAUDE.md
updated: 2026-08-09 (zwölfter Block archiviert, Rotation nach Zeile-20-Reboot-Nachweis — Abnahmematrix 20/20)
---
...
**Nachtrag, 2026-08-09, zweiter — Zeile 20 (Reboot) live bestanden, Matrix jetzt 20/20:**
Auftrag zu Sessionbeginn: Nikinger meldete einen soeben erfolgten Reboot, noch ohne eigenen
Handgriff danach — genau der passive Prüffall, den Zeile 20 vorsieht (wie P3 Zeile 6, siehe
`phase3_edge/CLAUDE.md` Korrekturnotiz 2026-07-29 — dort der Beleg-Maßstab, hier reproduziert,
nicht neu erfunden). Alles read-only erhoben, kein Handgriff:

- `uptime -s`/`who -b` → Boot `2026-08-09 11:40`.
- `sharefyx-mcp.service`: `is-enabled` → `enabled`; `ExecMainStartTimestamp` `11:40:20`, **0 s
  nach Boot**, ohne Nikinger-Zutun (`systemctl show`).
- `journalctl -u sharefyx-mcp -b`: „Started sharefyx-mcp.service" um `11:40:20`, App-Startup
  `11:40:23` — sauberer Autostart, kein manueller `start`/`restart` im Journal.
- **Connector, stärkster Beleg statt nur `401` gegen anonym:** `systemctl status` zeigt echten
  authentifizierten MCP-Traffic um `11:42:08` (`ListResourcesRequest`/`ListPromptsRequest`/
  `ListToolsRequest`, je Status 200) — der Dienst hat nach dem Reboot echte Arbeit gemacht.
  Zusätzlich, analog zum Beleg-Muster von Zeile 14: Claude Code selbst rief `list_spaces` über
  den echten, produktiven Connector auf (Space `niklas`) — `{"fabian": 6 items, ro}
  {"niklas": 40 items, rw}`, sauberer Read, keine Fehlermeldung, kein Re-Auth nötig.
- **UI:** lokal `GET /ui/login` (Port `8765`, aus der Unit-Definition, nicht dem falschen
  Default-Port) → `200`; öffentlich über dieselbe Funnel-URL wie in Zeile 6/`P3_ABNAHME` notiert
  (`savefyx-vmware-virtual-platform.tail89fc2a.ts.net`) → `200`. `/api/v1/me` und `/mcp/` ohne
  Auth → beide `401` (Isolation hält auch nach dem Reboot).
- **Tailscale/Funnel:** `tailscaled` `enabled`, `ExecMainStartTimestamp` identisch `11:40:20`;
  `tailscale funnel status` zeigt die Funnel-Konfiguration bereits wieder aktiv (`Funnel on`,
  Proxy auf `127.0.0.1:8765`) — die Serve-Config übersteht den Reboot selbst, ohne erneutes
  `tailscale funnel` von Hand.
- **Timer:** `systemctl is-enabled` → `enabled` für alle drei (`sharefyx-purge`,
  `sharefyx-backup`, `sharefyx-authbackup`); `list-timers` zeigt für alle drei ein gültiges
  `Next`-Feld (2026-08-10, korrekt neu vom systemd-Timer-Subsystem nach dem Boot berechnet).
  Kein Timer ist seit dem Boot gelaufen (`Last` liegt vor `11:40`) — das verlangt Zeile 20 auch
  nicht: „kommen zurück" heißt geladen/aktiv, nicht gefeuert; ein erzwungener Lauf wäre echte
  Live-Aktion und bewusst nicht ausgelöst. Zeile 6/12 in P3 trennten genau deshalb „kommt
  zurück" (Zeile 6) von „ist tatsächlich gelaufen" (eigene Zeile 12) — P5 fasst beides in einer
  Zeile zusammen, der Belegmaßstab bleibt aber „geladen/aktiv".

**Abnahmematrix damit vollständig: 20/20, 0 teilweise, 0 offen.** Root-`CLAUDE.md`/`ROADMAP.md`
bewusst noch **nicht** auf „Phase 5 ✅" gesetzt — das bleibt an die Step-9-Abschlussarbeiten
gebunden (siehe unten), nicht an die Matrix allein.

**Offen für die nächste Session:**
- Step-9-Abschlussarbeiten (kein Fabian mehr nötig, reine Doku/Runbook-Arbeit, jetzt
  vollständig entsperrt): altes `auth-users`-Credential/Keyring entfernen (Runbook-Schritt 4 —
  reale Infrastruktur, Nikinger), `P5_ABNAHME_<datum>.md`, `phase5_ui_uebersicht.svg`,
  `PHASE5_CLOSEOUT_HANDOVER.md`, Rotationsprüfung auf diesem Head (Softcap im Blick behalten),
  danach erst Root-`CLAUDE.md`/`ROADMAP.md` auf „Phase 5 ✅".
