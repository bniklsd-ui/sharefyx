...
| **P8.6** | `phase8_6_ui_polish/` | **🔄 Plan 1 + Plan 2 gebaut, Blöcke E + F + G + G-R abgeschlossen, nicht ausgeliefert** (Stand 2026-09-14). Step 0 ✅ · Step V ✅ (Ollama + `qwen3-vl:8b`; V-plugin ⚠️ gemessen und **zurückgebaut** — das Plugin amputiert M3s nativen Bildpfad) · Block A ✅ (Radiogruppe→`<select>`, sechs Layer-/Selektions-Tokens, `--border-soft`-Fix, Konvention v3 um „Vorsicht") · Block B ✅ (eine konsolidierte Hover-Regel, `.account-nav`, `.action--caution` mit genau zwei Trägern) · Block C ✅ **geliefert, nicht auslieferbar** (Rail-Reihenfolge, „Alle Items" unter die Spaces, `.overview` als Grid, klickbare Spaces, Ordner-Zähler) · Block D ✅ [D1/D2/D4] (V102-Dedup, FNV-1a-Layout-Seed, `cancelAnimationFrame`) · **Block E ✅** (`p86_viewport_probe.py` neu, CDP-Messung; **V125 geschlossen: 9a = 9b, gleiche Ursache, falsche Plan-Annahme** — bei 1200 px mit Banner byte-identische Maße auf main und v3.0.1; E2b gegen Wegwerf auf `6f19a8f` statt Produktion per Nikinger-Korrektur 2026-09-14) · **Block F ✅** (Layering: Meta-Panels entkoppelt von `--warn`, drei rohe Hex auf Token `--rail-top/--auth-glow/--auth-card-top`, zwei Wächter scharf — `test_no_raw_surface_hex_outside_root` + `test_meta_panel_is_not_tinted_with_the_warning_colour`; F2 null B-Aufwand; V127: 0) · **Block G ✅** (Layout-Umbau, **fünf Befunde in einem Schritt behoben**: `.shell` `240px 380px 1fr` → `240px 480px 1fr` per **P8.6-O2-Auslösung** N.7; DOM-Umzug `#list-overview` in `.list`-Slot + `#detail-graph` in `.detail`-Slot; `state.overview`-Flag neu; `.overview`-Grid + 1280-px-Query ersatzlos weg = Befund 9b-Ursache weg; `.detail__graph`-Höhenkette = V112-Wächter; `.overview__space-open` als Zeilen-Button mit Chips als `<span role="button">` = Befund 6; 4 neue statische Tests; `ui_budget` 5/5 mit 141 KB statt 130 KB — Plan §4.9-Erwartung „app.css kleiner" **nicht erfüllt**, dokumentierte Abweichung) · **Block G-R ✅** (Layout-Revision nach Sichtung, **vier Befunde in einem Schritt**: Breakpoints 1280→1200 mit Rail bleibt 240 + neuer 1024-er-Stapel; Layer-Tone-Drift behoben durch `.detail { background: var(--bg) }`; Editor-Header sticky + YAML bündig; 5 neue/angepasste Tests; `ui_budget` 5/5 mit 143 KB; zwischen G und H eingeschoben auf Nikinger-Vorgabe 2026-09-14) · **Gate ⬜ angehalten · Step Z ⬜ · Plan 2 §9 = kanonischer Closeout**. `pytest` **976**, Tabu-Diff §0.3 über die ganze Phase leer, keine neunte P1-Contract-Öffnung, Service-Touch 0. **Die Nikinger-Sichtprüfung vom 2026-09-12 hat neun UX-Befunde zurückgegeben statt der Freigabe** — kein Push, kein Deploy, Badge bleibt `v3.0.1`. Drei der Befunde öffnen gelockte Entscheidungen (P8.6-O2 `.shell`-Grid · C1/N3-Lesart b · P8.6-E); die Aufteilung 9a vs 9b war eine **unbewiesene Hypothese** (V125 geschlossen 2026-09-14 — beide sind Banner-getrieben und auf v3.0.1 byte-identisch). **Ein zweiter P8.6-Plan ist erforderlich**; Teil-Stand + Befund-Zuordnung: `docs/concepts/PHASE8_6_CLOSEOUT_HANDOVER.md`, Grafik: `docs/concepts/phase8_6_ui_polish_uebersicht.svg`. **[2026-09-13, Nikinger-Entscheidungen]** (a) Deploy-Ziel bleibt **`v3.0.2`**, P8.6-R bestaetigt; (b) **Block C geht einzeln raus** (Push), nicht in einem Sammel-Push nach Plan 2; (c) Ein-Block-Regel fuer die Wurzel-Current-state verworfen. **(d) Push ja, Deploy nein** — die Phasen-Commits sind am 2026-09-13 gepusht, ausgeliefert wird **nicht**; live bleibt `6f19a8f` (P8.5). **[2026-09-13, Korrektur]** Von P8.6 ist **nichts live** — auch Block A und D nicht: `/opt/sharefyx/current` zeigt auf den **P8.5**-Release `6f19a8f` vom 2026-09-05, sechs Block-Marker haben 0 Treffer live. Die Doku-Behauptung „Block A + D deployed, health_gate 8/8" war falsch; Das Werkzeug ist dabei in Ordnung: `health_gate.sh` liest den Release-SHA aus `/opt/sharefyx/current` (Z. 134/160); die Gegenprobe am 2026-09-13 meldet 7 OK + 1 FEHLER — das „8/8" ist nie so gelaufen. `deploy.sh` liefert `main` aus, einen Deploy „nur Block C" gibt es nicht. Herleitung: Handover §4.6. **[2026-09-13, Plan 2]** Die Planungssession hat stattgefunden: `docs/concepts/phase8_6_ui_polish_plan2.md` ist der maßgebliche Plan, der ausfuehrende Agent beginnt bei **Block E**. Neue Bloecke **E** (Befund 9 **messen**, nicht bauen — 9a live vs. 9b Block-C-eingefuehrt) · **F** (Layering: `--panel-meta-line` **ist** `--warn` bei 22 %) · **G** (Layout-Reorg, `.shell` → `240px 480px 1fr`, **P8.6-O2 ausgeloest und entschieden**; Editor ersetzt die Karte, **ESC** bringt sie zurueck) · **H** (Rail-Umkehr 7b, Abmelden bleibt aeusserster Knopf; `.account-nav`-Anmutung) · **J** (`pytest`-Flake, **erste datierte Tabu-Ausnahme** `phase4_auth/authserver/crypto.py` — `phase1_storage/storage/**` bleibt zu). Der **kanonische Closeout wandert nach Plan 2 §9** (P8.6-W), Plan 1 §9 bleibt leer. Baseline korrigiert: `pytest` **969 passed + 1 bekannter Flake**, nicht 970. | 🔄 |
