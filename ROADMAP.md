...
| **P8.6** | `phase8_6_ui_polish/` | **🔄 Step 0 ✅ + Step V 🟡-deferred + Block A ✅** (Stand 2026-09-10), Plan: `docs/concepts/phase8_6_ui_polish_plan.md`. Reichweite „Selektions-Welle + Layout" — Layering-Tokens, eine Selektionssprache für alles Klickbare, „Konto"→„Einstellungen", Ordner-Zähler, Rail-/Übersichts-Reorg, klickbare Spaces, drei Graph-Fixes (V102/§2.3/§2.4), Radiogruppe-Rückbau auf `<select>`. **Step V auf Vision-Backend** — OpenCode-Vision-Plugin-Installation **übersprungen** zugunsten eines **lokalen Modells** (`InternVL 2.5 8B` Q4 auf Ollama) auf einem **neu zu migrierenden Proxmox-Host** (i5-14600KF primär, danach Ryzen 7 5800X); 7-Schritte-Aktionsliste in `phase8_6_ui_polish/CLAUDE.md` §Vormerkungen dokumentiert (Nikinger-Aktion). **Block A Fundament ✅** (2026-09-10): A1 Radiogruppe→`<select>` (P8.6-H/I), A2 sechs neue Tokens (`--bg-void`/`--select-fill`/`--select-fill-quiet`/`--select-line`/`--select-line-quiet`/`--caution`) + fünf rohe `rgba(62,141,243,…)` ersetzt + `--bg-void` an genau drei Stellen (body, `.list__empty`, `.overview__graph-empty`, P8.6-D/E), A3 `--border-soft`→`var(--line)`-Renderfehler-Fix, A4 Konvention v3 um „Vorsicht" (`color: var(--caution)`); **+2 statische Tests**, `pytest` V107 ✅ 966 passed, `ui_budget` V97 ✅ 5/5 (130,4 KB). Block B (Selektion vereinheitlichen) als nächstes. **Zwei „would be cool"-Notes** (Nikinger 2026-09-10): Tab-Meta dynamisch (V120 offen), Custom 404-Seite (P8.6-Tabu §0.3 berührt, Folge-Phase). Deploy-Ziel `v3.0.2`. | 🔄 |
