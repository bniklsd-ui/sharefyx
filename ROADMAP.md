...
| **P8.6** | `phase8_6_ui_polish/` | **🔄 Step 0 ✅ + Step V ✅ + V-plugin ⚠️ (zurückgebaut) + V-vision-befund ✅ + Block A ✅ + Block B ✅ + Block C ✅ + Block D ✅ [D1/D2/D4]** (Stand 2026-09-12), Plan: `docs/concepts/phase8_6_ui_polish_plan.md`. Reichweite „Selektions-Welle + Layout" — Layering-Tokens, eine Selektionssprache für alles Klickbare, „Konto"→„Einstellungen", Ordner-Zähler, Rail-/Übersichts-Reorg, klickbare Spaces, drei Graph-Fixes (V102/§2.3/§2.4), Radiogruppe-Rückbau auf `<select>`. **Block A Fundament ✅** (2026-09-10): A1 Radiogruppe→`<select>` (P8.6-H/I), A2 sechs neue Tokens, A3 `--border-soft`→`var(--line)`, A4 Konvention v3 „Vorsicht"; **+2 statische Tests**, `pytest` 966 passed, `ui_budget` 130,4 KB. **Block B Selektion ✅** (2026-09-11): B1 konsolidierte Hover-Regel, B2 V113-Audit-Befund, B3 `.account-nav`-Klasse, B4 `.action--caution`-Trägerklasse (genau zwei Mitglieder), B5 `.link-picker-results` `6px`→`var(--radius-sm)`; **`pytest` 967**, `ui_budget` 133,1 KB; **vier Selbst-Screenshots** `p86_block_b_*.png`. **Block C Struktur ✅** (2026-09-12): C1 `#account-button` raus aus `.rail__account` + Label „Konto"→„Einstellungen" (N3-Lesart b); C2 `renderScopeRow()` wandert hinter die Spaces + `tree__group` „Alles"; C3 `.overview` wird zweispaltiges Grid (Wrapper-DIVs `head-row`/`col-left`/`col-right`, `.overview__graph` ohne `max-width`/`min-height` [V112-Gegenprobe], `@media (max-width: 1280px)` kollabiert); C4 `.overview__space-open` als innerer `<button>` mit B1-hover + V116 `activateView`-Export + `closeEditor`-Gating; C5 Folder-Zaehler aus `state.items` via `state.itemsLoaded` + V117-Reset in `activateView`/`navigateAll`; `requestAnimationFrame(resize)` in `loadGraph()` [V115]; **+3 statische Tests**, **`pytest` 970**, `ui_budget` 137,5 KB; **sechs Selbst-Screenshots** `p86_block_c_*.png`. **Block D ✅ [D1/D2/D4]** (2026-09-10): D1 V102-Dedup, D2 FNV-1a-Layout-Seed, D4 `cancelAnimationFrame`; **D3** wartete auf Block C, jetzt mit C3 zusammen umgesetzt (Grid + RAF). **Push + Deploy `v3.0.2`**: wartet auf Nikinger-Sichtung der Screenshots (Drei-Bedingungen-Regel). **Zwei „would be cool"-Notes** (Nikinger 2026-09-10): Tab-Meta dynamisch (V120 offen), Custom 404-Seite (P8.6-Tabu §0.3 berührt, Folge-Phase). | 🔄 |
