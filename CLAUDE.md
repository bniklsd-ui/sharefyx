...
**[2026-09-14, P8.6 Block E erledigt: E2a + E2b ✅ — opencode/M3 — Methodik-Wechsel auf Wegwerf-vor-Produktion.** **Reine Mess-Session, kein Produktcode-Touch.** E2b **nicht** gegen die echte Produktion, sondern gegen eine **zweite Wegwerf-Instanz auf v3.0.1-Stand** `6f19a8f` (der Release vom 2026-09-05, der live ist; `git worktree add -d /tmp/opencode/v301-worktree 6f19a8f`, venv-Symlink ins Repo-.venv, `wegwerf_setup_v3ritt.py cleanup+setup+seed-items+start` PID 162337, sauber per PID-Datei gestoppt, Worktree abgebaut). **Nikinger-Korrektur** statt E2b-gegen-Produktion: Wegwerf-vor-Produktion reicht für Befund-Reproduktion, ist exakt der Code der läuft, wahrt Hard Rule 1 (keine echten Creds) + Hard Rule 9 (kein Service-Touch) **vollständig**. **V125 geschlossen: 9a = 9b, gleiche Ursache, falsche Plan-Annahme** — bei 1200 px mit Banner sind die Maße auf main (Block C) und auf v3.0.1 **byte-identisch** (col-left 692×284, col-right 692×284, gestapelt); was Block C änderte, ist **welche** Recent-Items in welcher Reihenfolge erscheinen, **nicht** das Layout. Die wahre Ursache war **schon immer** die 140-px-Banner-Höhe und war schon auf v3.0.1 vorhanden — die Aufteilung in „9a (live) / 9b (Block-C-eingeführt)" war eine unbewiesene Hypothese, die nie gegen v3.0.1 verifiziert wurde. **V126 erledigt:0** unverändert. Sechs neue Screenshots `docs/screenshots/p86_probe_v3.0.1{,-clean}_{1024,1200,1440}.png` + zwei neue Probe-JSONs `phase8_6_ui_polish/probes/e2b_v3.0.1{,_clean}.json` (~65 KB je). **Selbstprüfung:** `pytest` 970 passed in 103,90 s (Baseline 969 + 1 Flake, Flake-Lauf diesmal nicht gezogen), `ui_budget` 5/5, Tabu-Diff §0.3 leer, sharefyx-mcp PID 991 nur gelesen, Wegwerf-Instanz PID 162337 sauber per PID-Datei gestoppt. **Doku-Hygiene:** Modul-Status Z10 🟡→✅, Phase-Head-`updated:` mit E2b-Eintrag ergänzt, SESSIONS_ARCHIVE.md-`updated:` mit Rotations-Eintrag, ROADMAP-P8.6-Zeile + docs/INDEX.md Phase-8.6-Karten nachgezogen; Rotation per `scripts/rotate_session_block.sh phase8_6_ui_polish`, E2a-Sub-Block (78 Z. / 6.720 B) verbatim ins Archiv, Head 74.518 → 67.798 B. `docs/INDEX.md` 38.830 B / **82 B Reserve** zum 38-KB-Softcap. **Naechster Schritt: Block F (Layering, Befunde 1+8) — reine Token-Arbeit, unabhängig von Produktion.**
