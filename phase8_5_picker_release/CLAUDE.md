...
nicht selbst gesehen. **Sichtung als nächste Session in Phase 8.5** (Nikinger-Entscheidung
2026-09-08, scope-reversal: NICHT nach P8.6, sondern Phase 8.5 hier abschließen), kein
neuer Code nötig — nur eine Sichtung der bereits vorliegenden Block-C-Belege gegen die neue
Regel. Konkrete Anleitung: `SICHTPRUEFUNG_WALKTHROUGH.md` (freundlich) +
`SICHTPRUEFUNG_RESTBLOCK.md` (technisch); Reihenfolge im Restblock §Reihenfolge-Empfehlung.
...
## Nächste Session (Nikinger-Entscheidung 2026-09-08, scope-reversal)

**Was als nächstes in Phase 8.5 läuft (NICHT P8.6 — Nikinger-Entscheidung 2026-09-08):**
Sichtung der 10 P8.5-🟡-Zeilen gegen die am selben Tag geänderte Statusregel
(Nikinger-geprüfte Wegwerf-Instanz zählt als live-verifiziert). Belege liegen alle seit
Block C (2026-09-04) in den Screenshots `docs/screenshots/v3ritt_{chromium,firefox}_*.png`
vor — kein neuer Code, nur Sichtung. Bei ✅-Sprung: Bilanz wird 20 ✅ · 0 🟡 · 0 ⬜ von 20,
Phase 8.5 ist dann formal vollständig abgeschlossen (nicht nur "10 ✅ unter neuer Regel +
10 🟡 als Kandidaten").

**Die 10 Zeilen, einzeln:**
- **P8.5-6** — Modus „Text-Link" mit `[`/`]`-Escape; D4-Fund-Bracket-Fix ist seit 2026-09-07 committet, jetzt Sichtung am echten v3.0.1 nötig
- **P8.5-7** — Modus „Kante": `#field-links` enthält `itm_…`, Textarea byte-identisch
- **P8.5-8** — `localStorage`-Persistenz über Schließen+Wiederöffnen (privater Modus nicht direkt getestet)
- **P8.5-9** — `insertAtCursor` genau einmal auf Modulebene (statischer Test grün, Sichtung am Echten)
- **P8.5-10** — `ArrowDown`/`ArrowUp` + `aria-activedescendant` + Cursor klemmt
- **P8.5-11** — Neu-Tippen Cursor-Reset + „Keine Treffer." nie auswählbar
- **P8.5-12** — Tastatur-/Maus-Pfad beide durch `_pickLinkPickerAt`, `app.js` unverändert
- **P8.5-13** — V101 in Chromium **und** Firefox beantwortet
- **P8.5-14** — `app.css` genau ein Auswahl-Block, kein totes `:focus`
- **P8.5-15** — v3-Vorabritt 13/13 Stationen in beiden Browsern, drei echte Befunde vorgelegt

**Werkzeug:** `SICHTPRUEFUNG_WALKTHROUGH.md` (Was-tust-du / Was-siehst-du pro Schritt,
freundlich), `SICHTPRUEFUNG_RESTBLOCK.md` (technisch mit Tabellen + Code-Ankern), Login-Snippet
mit re-runnablem TOTP-Code am Dateiende des Walkthroughs. Setup-Stand: Production
v3.0.1 live (PID 355956 unverändert); die zwei Wegwerf-Instanzen (200-Knoten auf Port 18772,
D2-14-Knoten auf Port 18768) wurden am 2026-09-08 abgebaut — bei Bedarf neu starten
mit `phase8_ui_graph/scripts/wegwerf_setup_{200knoten,d2}.py setup,seed-items,start`
(Hard Rule 9-konform via PID-Datei, niemals `pkill -f`).

**Erwarteter Effekt:** Phase 8.5 springt von 10 ✅ auf 20 ✅; alle Block-C-Evidenz ist
damit unter der neuen Statusregel sichtgeprüft. Kein neuer Code, kein Deploy nötig —
reine `phase8_5_picker_release/CLAUDE.md` §Abnahmestand-Update + ggf. kleiner
INDEX/ROOT-Frontmatter-Touch im selben Commit.
